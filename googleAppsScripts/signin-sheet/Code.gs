...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Hazard photo review */
  .hazard-card { background: #fff; border-radius: 10px; box-shadow: var(--shadow); overflow: hidden; margin-bottom: 16px; animation: rise 0.4s ease both; border-left: 4px solid #c0392b; }
  .hazard-card-header { padding: 14px 28px; border-bottom: 1px solid var(--line); display: flex; align-items: center; gap: 10px; }
  .hazard-icon { color: #c0392b; flex-shrink: 0; }
  .hazard-card-title { font-size: 11px; font-weight: 500; letter-spacing: 0.12em; text-transform: uppercase; color: #c0392b; }
  .hazard-card-subtitle { font-size: 12px; color: var(--ink-muted); margin-left: auto; }
  .hazard-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; padding: 18px 28px 22px; }
  .hazard-thumb { position: relative; border-radius: var(--radius); overflow: hidden; background: var(--paper-mid); cursor: pointer; aspect-ratio: 4/3; }
  .hazard-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.2s, opacity 0.2s; }
  .hazard-thumb:hover img { transform: scale(1.04); opacity: 0.88; }
  .hazard-thumb-caption { position: absolute; bottom: 0; left: 0; right: 0; padding: 5px 8px; background: linear-gradient(transparent, rgba(0,0,0,0.65)); color: #fff; font-size: 11px; line-height: 1.3; }
  .hazard-thumb-num { position: absolute; top: 6px; left: 6px; background: rgba(192,57,43,0.85); color: #fff; font-size: 11px; font-weight: 500; width: 22px; height: 22px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
  .hazard-loading { padding: 28px 28px; color: var(--ink-faint); font-size: 13px; text-align: center; }
  /* Lightbox */
  .lb-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.88); z-index: 500; align-items: center; justify-content: center; padding: 20px; }
  .lb-overlay.open { display: flex; }
  .lb-inner { position: relative; max-width: min(90vw, 900px); max-height: 90vh; display: flex; flex-direction: column; align-items: center; gap: 10px; }
  .lb-img { max-width: 100%; max-height: calc(90vh - 60px); border-radius: var(--radius); object-fit: contain; display: block; }
  .lb-caption { color: rgba(255,255,255,0.75); font-size: 13px; text-align: center; }
  .lb-nav { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.15); border: none; color: #fff; font-size: 22px; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: background 0.15s; }
  .lb-nav:hover { background: rgba(255,255,255,0.28); }
  .lb-prev { left: -54px; }
  .lb-next { right: -54px; }
  .lb-close { position: absolute; top: -40px; right: 0; background: none; border: none; color: rgba(255,255,255,0.7); font-size: 28px; cursor: pointer; line-height: 1; padding: 0; }
  .lb-close:hover { color: #fff; }
  .lb-counter { position: absolute; top: -36px; left: 0; color: rgba(255,255,255,0.5); font-size: 12px; }
  @media (max-width: 560px) {
    ...
    .hazard-card-header, .hazard-grid { padding-left: 18px; padding-right: 18px; }
    .hazard-grid { grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); }
    .lb-prev { left: -44px; }
    .lb-next { right: -44px; }
  }
  .gas-version-pill {
...
<div class="page">
  ...
  <!-- Hazard Photo Review -->
  <div class="hazard-card" id="hazard-card" style="display:none">
    <div class="hazard-card-header">
      <svg class="hazard-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
      <span class="hazard-card-title">Hazard Identification — Review Before Signing</span>
      <span class="hazard-card-subtitle" id="hazard-count"></span>
    </div>
...
<script>
  ...
  function loadHazardPhotos() {
    google.script.run
      .withSuccessHandler(function(photos) {
        ...
        var card = document.getElementById('hazard-card');
        var grid = document.getElementById('hazard-grid');
        var countEl = document.getElementById('hazard-count');
        countEl.textContent = _hazardPhotos.length + ' photo' + (_hazardPhotos.length !== 1 ? 's' : '');
        grid.innerHTML = _hazardPhotos.map(function(p, i) {
          var thumb = driveThumbUrl(p.url);
          return '<div class="hazard-thumb" onclick="lbOpen(' + i + ')">' +
            '<img src="' + thumb + '" alt="" loading="lazy">' +
            '<div class="hazard-thumb-num">' + (i + 1) + '</div>' +
            (p.caption ? '<div class="hazard-thumb-caption">' + p.caption + '</div>' : '') +
            '</div>';
        }).join('');
  ...
  function lbRender() {
    ...
    document.getElementById('lb-img').src = driveThumbUrl(p.url);
